...
- **2026-07-24 (patch v1, primer parche REAL)** — Publicado: `Main.exe` con Triple Shot v2 (targeting posicional), HUD contador de flechas (estaba fuera de pantalla en x=-29) y fixes beta. Pipeline completo OK: promote-to-release (plumbing) → make-patch (FullPatch==Patch1, 3.537.366 B, crc `602c1112`, 27.262 archivos hasheados ~4min en WSL) → zips a R2 (subida manual dashboard; CRC verificado post-upload con descarga real) → version.txt (webapp-rex `9f983a0`) → tag `client-patch-1`. Convención nueva: `patch-manifests/version-v<N>.txt` guarda copia del manifest publicado (es el `--prev-manifest` del v(N+1); sin él, el pipeline v2 dependería de bajar el de Vercel). E2E formal §3 (4 casos) NO corrido esta vez (decisión beta) — el update real hizo de test. **RESULTADO: el updater funcionó a la perfección en producción** (camino full v0→v1: detección de latest, descarga, verificación CRC, aplicación de Main.exe, bump de Version — confirmado por el operador tras el deploy). El camino INCREMENTAL (patch.N encadenado) sigue sin ejercitarse en prod — se prueba solo a partir del v2 con clientes ya en v1; primer candidato a vigilar en el próximo parche. Gaps §1.6 (a/b, detección proactiva de Main.exe corriendo + staging atómico) siguen pendientes y NO se manifestaron en este lanzamiento.
